...
function applyTrailMaterialStability(material) {
    if (!material) return;
    if ('alphaToCoverage' in material && material.alphaToCoverage) {
        material.alphaToCoverage = false;
    }
}

function createTrailMaterial(color, effectiveWidth, effectiveOpacity, useWideLine = false) {
    if (!useWideLine) {
        ...
        applyTrailMaterialStability(material);
        return material;
    }
...
export class StraightLineTrail {
    ...
    constructor(scene, color = TRAIL_COLOR, lineWidth = TRAIL_LINE_WIDTH, maxSegments = TRAIL_MAX_SEGMENTS, opacity = TRAIL_OPACITY, minSegmentDistance = 0) {
        ...
        this._material = createTrailMaterial(this._color, effectiveWidth, effectiveOpacity, this._useWideLine);
    ...
    update(pos) {
        ...
        if (!this._currentDir) {
        ...
        } else if (dir.dot(this._currentDir) > TRAIL_DIRECTION_DOT_THRESHOLD) {
            ...
            const maxVertexCapacity = this._useWideLine
                ? Math.floor(this._positions.length / 3)
                : this._attr.count;
            if (this._vertexCount >= maxVertexCapacity) return;
...
export class SegmentTrailBatch {
    constructor(scene, capacity = 1, color = TRAIL_COLOR, lineWidth = TRAIL_LINE_WIDTH, opacity = TRAIL_OPACITY) {
        ...
        const effectiveOpacity = scaleOpacityForDisplay(this._opacity);
        const effectiveWidth = scaleLineWidthForDisplay(this._lineWidth);
        this._material = createTrailMaterial(color, effectiveWidth, effectiveOpacity, true);
...
export function mergeTrailsIntoLineSegments(trails, scene, color = TRAIL_COLOR, lineWidth = TRAIL_LINE_WIDTH, opacity = TRAIL_OPACITY, options = null) {
    ...
    let geometry;
    let merged;
    const useWideLine = (options && typeof options.useWideLine === 'boolean')
        ? options.useWideLine
        : USE_WIDE_TRAIL_LINES_MERGED;
    if (useWideLine) {
        geometry = new LineSegmentsGeometry();
        geometry.setPositions(positions);
        const material = createTrailMaterial(color, effWidth, effOpacity, true);
        ...
        const material = createTrailMaterial(color, effWidth, effOpacity, false);
        if (options && Number.isFinite(options.fadeInMs) && options.fadeInMs > 0) {
            applyFadeIn(material, effOpacity, options.fadeInMs);
        }
...
export function buildMergedLineSegmentsFromSegments(segmentsList, scene, color = TRAIL_COLOR, lineWidth = TRAIL_LINE_WIDTH, opacity = TRAIL_OPACITY, options = null) {
    ...
    if (useWideLine) {
        geometry = new LineSegmentsGeometry();
        geometry.setPositions(positions);
        const material = createTrailMaterial(color, effWidth2, effOpacity2, true);
        if (options && Number.isFinite(options.fadeInMs) && options.fadeInMs > 0) {
        ...
        geometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
        geometry.setDrawRange(0, positions.length / 3);
        const material = createTrailMaterial(color, effWidth2, effOpacity2, false);
        if (options && Number.isFinite(options.fadeInMs) && options.fadeInMs > 0) {
            applyFadeIn(material, effOpacity2, options.fadeInMs);
        }
        merged = new THREE.LineSegments(geometry, material);
