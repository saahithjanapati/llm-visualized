...
function createStaticConnectorLine({
    ...
    const end = new THREE.Vector3(x, startY, z);
    ...
    line.userData.connectorStartY = startY;
    line.raycast = () => {};
    return line;
}

function updateConnectorEndY(connector, endY) {
    if (!connector || !connector.geometry) return;
    const points = connector.geometry.attributes?.position;
    if (!points || !points.array || points.array.length < 6) return;
    const startY = Number.isFinite(connector.userData?.connectorStartY)
        ? connector.userData.connectorStartY
        : points.array[1];
    const clampedEndY = Math.max(startY, Number.isFinite(endY) ? endY : startY);
    points.array[4] = clampedEndY;
...
export function addEmbeddingAndTokenChips({
    ...
    try {
        ...
        const slotIndices = Array.isArray(laneLayoutIndices) && laneLayoutIndices.length
            ? laneLayoutIndices.slice(0, safeLaneCount)
            : Array.from({ length: safeLaneCount }, (_, idx) => idx);
        while (slotIndices.length < safeLaneCount) {
            slotIndices.push(slotIndices.length);
        }
        const computedLayoutCount = Number.isFinite(laneLayoutCount)
            ? Math.max(safeLaneCount, Math.floor(laneLayoutCount))
            : Math.max(
                safeLaneCount,
                slotIndices.reduce(
        ...
        const laneSpacing = LN_PARAMS.depth / (computedLayoutCount + 1);
        const laneZs = slotIndices.map((laneIdx) => {
        ...
        const resolveTokenIndexForLane = (laneIdx, tokenIndexList = null) => {
            ...
            return Math.max(0, Math.floor(laneIdx));
        };
        const resolveTokenIdForIndex = (tokenIndex) => {
            if (!activationSource || typeof activationSource.getTokenId !== 'function') return null;
            return activationSource.getTokenId(tokenIndex);
        };
        const applyChipSelectionMetadata = (
            chip,
            { chipLabel, laneIndex, tokenIndex, tokenId, tokenLabel, positionIndex }
        ) => {
            chip.userData = chip.userData || {};
            chip.userData.label = chipLabel;
            chip.userData.laneIndex = laneIndex;
            chip.userData.tokenIndex = tokenIndex;
            chip.userData.positionIndex = positionIndex;
            if (typeof tokenLabel === 'string') chip.userData.tokenLabel = tokenLabel;
            if (Number.isFinite(tokenId)) {
                chip.userData.tokenId = Math.floor(tokenId);
            } else if ('tokenId' in chip.userData) {
                delete chip.userData.tokenId;
            }
            chip.name = chipLabel;
        };
        ...
        if (lastLayer && lastLayer.mlpDown && lastLayer.mlpDown.group) {
            ...
            let topLogitRevealStarted = false;
            const clearTopLogitRevealTimeout = () => {
            ...
            const startTopLogitReveal = ({ immediate = false } = {}) => {
            ...
            if (topLogitBars && pipeline && typeof pipeline.addEventListener === 'function') {
                ...
                const maybeReveal = () => {
                    ...
                    const immediate = typeof pipeline.isSkipToEndActive === 'function'
                        ...
                        : false;
                    if (immediate) {
                        return startTopLogitReveal({ immediate: true });
                    }
                    if (topLogitRevealStarted || topLogitRevealTimeoutId) {
                        return topLogitRevealStarted;
                    }
                    topLogitRevealTimeoutId = setTimeout(() => {
                        topLogitRevealTimeoutId = null;
                        if (disposed) return;
                        const revealImmediate = typeof pipeline?.isSkipToEndActive === 'function'
                            ? pipeline.isSkipToEndActive()
                            : false;
                        startTopLogitReveal({ immediate: revealImmediate });
                    }, TOP_LOGIT_REVEAL_DELAY_MS);
                    return false;
                };
                const onProgress = () => {
                    if (topLogitRevealStarted || maybeReveal()) {
                        pipeline.removeEventListener('progress', onProgress);
                        if (topLogitProgressHandler === onProgress) {
                            topLogitProgressHandler = null;
                        }
                    }
                };
    ...
    const dispose = () => {
        ...
        if (typeof pipeline?.setTopLogitRevealPending === 'function' && typeof pipeline?.setTopLogitRevealComplete === 'function') {
            pipeline.setTopLogitRevealPending(false);
        }
