...
describe('SelectionPanel 2D canvas selection resolution', () => {
    ...
    it('preserves overview attention-head canvas selections as label selections', () => {
        const panel = createPanelContext();
        const selection = {
        ...
        expect(resolved?.label).toBe('Attention Head 3');
        expect(resolved?.kind).toBe('label');
        expect(resolved?.info?.layerIndex).toBe(1);
        expect(resolved?.info?.headIndex).toBe(2);
        expect(resolved?.info?.activationData?.stage).toBe('attention.head');
    });

    it('preserves overview MLP canvas selections as label selections', () => {
        const panel = createPanelContext();
        const selection = {
        ...
        expect(resolved?.label).toBe('Multilayer Perceptron');
        expect(resolved?.kind).toBe('label');
        expect(resolved?.info?.layerIndex).toBe(4);
        expect(resolved?.info?.activationData?.stage).toBe('mlp');
    });
});
